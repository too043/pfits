	...
	if (plotType==1)
	  {
		...
		for (i=i0;i<i1;i++)
		  {
		    for (j=0;j<nbin;j++)
		      {
			if (tt==1)
			  {minVal_timeAve = maxVal_timeAve = timeAve[(i-kk0)*nbin+j]; tt=0;}
			else if (minVal_timeAve > timeAve[(i-kk0)*nbin+j]) minVal_timeAve = timeAve[(i-kk0)*nbin+j];
			else if (maxVal_timeAve < timeAve[(i-kk0)*nbin+j]) {maxVal_timeAve = timeAve[(i-kk0)*nbin+j]; maxValChan = (i);}
		      }
		  }
	      }
	    //	    maxVal_timeAve = 4000;
	    ...
	    tr[0] = 0;  tr[1] = 1;  tr[2] = 0;
	    tr[3] = kk0;  tr[4] = 0;  tr[5] = 1;
      

	    cpgimag(timeAve,nbin,nchan-kk0,1,nbin,1,nchan-kk0,minVal_timeAve,maxVal_timeAve,tr);
	    if (helpFindBright==1)
	      {
		fx[0] = minx; fx[1] = maxx;
      ...
      for (kk=kk0;kk<kk1;kk++)
	{
	  ...
	  if (datWts[ii*nchan+kk] > 0)
	      ...
	      for (jj=0;jj<nbin;jj++)
		{
		  ...
		  timeAve[(kk-kk0)*nbin+dedispBin] += (loadAll_pol1[(ii)*nbin*nchan+kk*nbin+jj]+loadAll_pol2[(ii)*nbin*nchan+kk*nbin+jj]);
		  freqAve[(ii-ii0)*nbin+dedispBin] += (loadAll_pol1[(ii)*nbin*nchan+kk*nbin+jj]+loadAll_pol2[(ii)*nbin*nchan+kk*nbin+jj]);
		  //		  if (selectSB == 1)
