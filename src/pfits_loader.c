 ...
 */
void pfits_read1pol_float(float *out,int polNum,dSetStruct *dSet,float t1,float t2,int rangeType,long *nSamples,int *nTimeSamples,int *nFreqSamples,int debugFlag,int offsScl)
{
  int nchan;
  int nbits;
  int npol;
  ...
  if (dSet->headerMemorySet==1 && dSet->fileOpen==1)
    {
      ...
      datOffs = (float *)malloc(sizeof(float)*nchan);
      datScl = (float *)malloc(sizeof(float)*nchan);	
