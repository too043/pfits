...
void processHDU(fitsfile *fp,int hduNum,int iomode);
...
int main(int argc,char *argv[])
{
  ...
  if (quick==1)
    ...
    {
      fits_open_file(&fp,fname,iomode,&status);
      fits_get_num_hdus(fp,&nhdus,&status);
      ...
      do 
	{
	  ...
	  if (iomode == READONLY)
	    ...
	    printf("Enter value of HDU to view (or 'c' to change, '0' to quit and save): ");
	  scanf("%s",input);
	  if (input[0]=='c')
	    {
	      char label[1024];
	      ...
	      printf("Enter parameter label ");
	      scanf("%s",label);
	      fits_movabs_hdu(fp,hduNum,&hdutype,&status);
	      if (fits_read_card(fp,label,card,&status))
	      ...
	      else
		{
		  ...
		  if (*card && fits_get_keyclass(card) == TYP_STRUC_KEY)
		      ...
		      if (*comment)
			{
		      ...
		      fits_update_card(fp,label,card,&status);
		      printf("%s\n",card);
		    }
		}
	      //	      hduNum=1;
	    ...
	    sscanf(input,"%d",&hduNum);

	  if (hduNum > 0) processHDU(fp,hduNum,iomode);
	} while (hduNum > 0);
      printf("\n\n Goodbye\n");
      fits_close_file(fp,&status);
    }
  ...
  if (hduNum > 1)
      ...
      do
	{
	  ...
	  if (iomode == READONLY)
	    ...
	    printf("Select data to view (-1 to return, 'c' = change) ");
	  scanf("%s",input);
	  if (input[0]=='c')
	    {
	      char label[1024];
